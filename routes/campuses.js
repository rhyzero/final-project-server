...
router.delete(
  ...
  ash(async (req, res) => {
    let campus = await Campus.findByPk(req.params.id, { include: [Student] });
    let students = campus.students;
    var index = students.findIndex(function (e) {
      return e.id == req.params.studentId;
    });
    if (index !== -1) campus.students.splice(index, 1);
...
router.get(
  ...
  ash(async (req, res) => {
    ...
    res
      .status(200)
      .json(students.find((student) => student.id == req.params.studentId));
  })
);
